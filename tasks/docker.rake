task :docker do
  ...
  runcmd '( iptables-save -t nat | grep -i \\\-A | grep -i docker | sed s/-A/-D/ | xargs -t -L1 iptables -t nat ) || true'
  runcmd '( iptables-save -t filter | grep -i \\\-A | grep -i docker | sed s/-A/-D/ | xargs -t -L1 iptables -t filter ) || true'
